...
from nnunetv2.training.lr_scheduler.cosine import CosineAnnealingScheduler
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        if os.environ.get('MODEL_NAME') == 'smdnet':
            self.initial_lr = 3e-4
            self.weight_decay = 1e-4
            self.num_epochs = 600
            self.lambda_v = float(os.environ.get('SMDNET_LAMBDA_V', '1.0'))
            self.lambda_a = float(os.environ.get('SMDNET_LAMBDA_A', '1.0'))
        else:
            self.initial_lr = 1e-2
            self.weight_decay = 3e-5
            self.num_epochs = 1000
            self.lambda_v = self.lambda_a = 1.0
        self.oversample_foreground_percent = 0.33
        self.num_iterations_per_epoch = 250
        self.num_val_iterations_per_epoch = 50

        if os.environ.get('SMDNET_SMOKE', '').lower() in ('1', 'true', 'yes'):
            self.num_epochs = 1
            self.num_iterations_per_epoch = int(os.environ.get('SMDNET_SMOKE_ITERS', '2'))
        ...
        self.save_every = 50
        self.disable_checkpointing = os.environ.get('SMDNET_SMOKE', '').lower() in ('1', 'true', 'yes')
    ...
    def _build_loss(self):
        if self.label_manager.has_regions:
            ...
            if os.environ.get('MODEL_NAME') == 'smdnet':
                ...
                focal_kwargs = {'gamma': 2.0, 'alpha': 0.25, 'reduction': 'mean'}
                aneurysm_loss = DC_and_Focal_loss(
                    dice_kwargs, focal_kwargs,
                    weight_focal=1, weight_dice=1,
                    ignore_label=self.label_manager.ignore_label,
                    dice_class=MemoryEfficientSoftDiceLoss
                )
                vessel_loss = DC_and_Focal_loss(
                    {'batch_dice': self.configuration_manager.batch_dice,
                     'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp},
                    focal_kwargs,
                    weight_focal=1, weight_dice=1,
        ...
        if os.environ.get('MODEL_NAME') == 'smdnet' and not self.label_manager.has_regions:
            aneurysm_ds = DeepSupervisionWrapper(aneurysm_loss, weights)
            loss = SMDNetMultiTaskLoss(
                aneurysm_ds, vessel_loss,
                lambda_v=self.lambda_v, lambda_a=self.lambda_a
            )
            self.print_to_log_file(
                f'SMD-Net loss: L = λ_v L_vessel + λ_a L_aneurysm with λ_v={self.lambda_v}, λ_a={self.lambda_a}, '
                f'Dice+Focal(γ=2.0)'
            )
        else:
    ...
    def configure_optimizers(self):
        if os.environ.get('MODEL_NAME') == 'smdnet':
            ...
            lr_scheduler = CosineAnnealingScheduler(optimizer, self.initial_lr, self.num_epochs)
        else:
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            if os.environ.get('MODEL_NAME') == 'smdnet':
                fine, coarse = self.network(data, return_coarse=True)
                l = self.loss(fine, coarse, target)
            else:
    ...
    def save_checkpoint(self, filename: str) -> None:
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                ...
                if os.environ.get('MODEL_NAME') == 'smdnet':
                    state = {k: v for k, v in state.items() if not k.startswith('sam_image_encoder.')}
                checkpoint = {
    ...
    def run_training(self):
        ...
        if not (os.environ['MODEL_NAME'] == 'nnunet' or os.environ['MODEL_NAME'] == 'nnsam' or os.environ['MODEL_NAME'] == 'nnunet3d' or os.environ['MODEL_NAME'] == 'smdnet'):
